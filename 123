...
# 生成大素数和原根
def generate_large_prime_and_primitive_root():
    primes = list(primerange(10**5, 10**7))
    p = random.choice(primes)
    g = random.choice([x for x in range(2, p) if isprime(x)])
    return p, g
...
def diffie_hellman_key_exchange(p, g, a, b):
    A = pow(g, a, p)
    B = pow(g, b, p)
    session_key_A = pow(B, a, p)
    session_key_B = pow(A, b, p)
...
if __name__ == "__main__":
    p, g = generate_large_prime_and_primitive_root()
    print(f"大素数： {p}")
    print(f"源根： {g}")
    a, b = random.randint(1, p-1), random.randint(1, p-1)
    session_key = diffie_hellman_key_exchange(p, g, a, b)
    key = str(session_key)
    print(f"DH密钥： {key}")
    iv = "iviviviv"
    md = MyDES()
    cbc_encode = md.encode_by_cbc('i am syf', des_key=key, iv=iv)
    print(f"加密消息： {cbc_encode}")
    print(f"解密消息： {md.decode_by_cbc(cbc_encode, des_key=key, iv=iv)}")
